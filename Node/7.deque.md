...
vector 是单向开口的连续线性空间，deque 则是一种双向开口的连续线性空间。所谓双向开口，意思是可以在常数时间内在头尾两端分别做元素的插人和删除操作，vector 当然也可以在头尾两端进行操作（从技术观点），但是其头部操作效率奇差，无法被接受。

deque和vector 的最大差异，一在于 deque 允许于常数时间内对起头端进行元素的插人或移除操作，二在于 deque 没有所谓容量(capacity）观念，因为它是动态地以分段连续空间组合而成，随时可以增加一段新的空间并链接起来。换句话说，像vector 那样“因旧空间不足而重新配置一块更大空间，然后复制元素，再释放旧空间”这样的事情在 deque 是不会发生的。也因此，deque 没有必要提供所谓的空间保留（reserve）功能。

虽然 deque 也提供 Raidon Access Hterator， 但它的迭代器并不是普通指针，其复杂度和 vector 不可以道里计（稍后看到源代码，你便知道），这当然影响了各个运算层面。因此，**除非必要，我们应尽可能选择使用 vector 而非 deque。**对deque 进行的排序操作，为了最高效率，可将 deque 先完整复制到一个 vector身上，將 vector 排序后（利用 STL sort 算法），再复制回 deque。

**在对deque的进行元素的插入删除之后，由于进行了元素的移动，原有的迭代器失效。**
...
struct __deque_iterator {
    ...
    void set_node(map_pointer new_node) {
        node = new_node;
        first = *new_node;
        last = first + difference_type(buffer_size());		//从这里可以看出，last指针指向的是缓冲区的结尾，如下图所示
    }
};

```

- **`deque`的迭代器中`last`指针永远指向`node`所指缓冲区的结尾。**
- **除首尾两个缓冲区有空闲的备用空间外，中间的缓冲区可以完全"满存"元素，即512字节的缓冲区可以存128个4字节的数据。**

<img src="7.deque.assets/截屏2023-05-21 10.48.38.png" alt="截屏2023-05-21 10.48.38" style="zoom:50%;" />
...
这行代码非常巧妙，它兼顾了`node`和`x.node`指向同一个节点和指向不同节点的情况。

- 情况一：`node == x.node`

  <img src="7.deque.assets/截屏2023-05-21 15.33.42.png" alt="截屏2023-05-21 15.33.42" style="zoom:50%;" />

  把相减的结果设为`result`,可以看出，`(cur - first)+(x.last - x.cur) = buffer_size() +result;`由上面的代码刚好可以得到结果`result`

- 情况二：`node != x.node`

<img src="7.deque.assets/截屏2023-05-21 15.34.29.png" alt="截屏2023-05-21 15.34.29" style="zoom:50%;" />
...
<img src="7.deque.assets/截屏2023-05-21 14.20.29.png" alt="截屏2023-05-21 14.20.29" style="zoom: 33%;" />

## deque的构造和内存管理

`deque`构造函数需要把`deque`的结构安排。也就是要设置迭代器，中控器和缓冲区。
...
**deque的插入删除操作需要移动元素，所以在删除操作之后，原有的迭代器失效。需要重新根据`begin()`或`end()`来操作迭代器。**

在插入时要判断是否需要创建新的缓冲区或使用新的map

- `push_back()`需要判断`finish`指向的缓冲区是否有一个以上的元素备用空间（`last`指向缓冲区的末尾，并不是最后一个节点）。即`finish.cur != finish.last -1`。

- `push_front()`需要判断`start`指向的缓冲区是否有一个以上的元素备用空间，即`start.cur != start.first`

在删除元素时，需要判断

- 移动删除位置之前的所有元素，还是移动删除位置之后的全部元素
- 删除后是否需要释放空闲的缓冲区
...
在数据中间插入数据时调用`insert_aux()`。也需要比较插入点前后哪部分元素少，就移动哪部分。

以移动前一部份元素为例：

<img src="7.deque.assets/截屏2023-05-21 16.48.43.png" alt="截屏2023-05-21 16.48.43" style="zoom:50%;" />
